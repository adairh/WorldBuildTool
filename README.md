...
Generated artifacts and exports are written to the directory configured by
`STORAGE_DIR` (defaults to `./storage`).

Optional visualization helpers (Leafmap, Plotly) are included in the default
requirements. If you prefer a leaner install you can omit them; the UI will
fall back to textual summaries when those packages are unavailable.
...
The project intentionally avoids Docker to keep the workflow lightweight and
Python-first. Use virtual environments to isolate dependencies and run the API
and UI processes directly from your workstation.
