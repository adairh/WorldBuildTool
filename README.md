...
ui/
  ...
  components/        # world.py, map.py, people.py, timeline.py, quests.py, assets.py
...
## Mở rộng với ChatGPT (tùy chọn)

Nếu cài đặt package `openai` và set `OPENAI_API_KEY`, hàm `craft_quest` sẽ gọi ChatGPT để gợi ý quest beat. Không có key thì hệ thống dùng generator deterministic dựa trên seed.
